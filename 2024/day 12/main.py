class Solution:
    def parse_data(self, data):
        ...
        coords_by_type = {}
        rows = len(data)
        cols = max(len(line.strip()) for line in data)
    ...
    def part1(self, data):
        ...
        for _type in types:
            ...
            while coords:
                ...
                connected_count = (
                    {}
                )  # count of connected sides for each cell in the group
                for coord in curr_group:
                    connected_count[coord] = get_connected_count(grid, coord)

                price = len(curr_group) * (
                    len(curr_group) * 4 - sum(connected_count.values())
                )
                prices[_type] += price
            ...
            for _ in range(2):
                for y in range(1, rows + 1):
                    ...
                    sides += len(
                        "".join(
                            [
                                "X" if current != above and current == "X" else " "
                                for current, above in zip(grid[y], grid[y - 1])
                            ]
                        ).split()
                    )
...
if __name__ == "__main__":
    ...
    # Call part1 and part2 and print the results
